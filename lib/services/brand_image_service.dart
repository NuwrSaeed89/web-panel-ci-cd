...
class BrandImageService {
  ...
  /// رفع صورة مزود إلى Firebase Storage
  static Future<String> uploadBrandImage({
    ...
    try {
      if (kDebugMode) {
        print('🔄 Starting brand image upload...');
        print(
      ...
      final finalFileName = fileName ?? 'brand_$timestamp.jpg';
      final storagePath = 'brands/$finalFileName';
  ...
  /// رفع صورة من XFile
  static Future<String> uploadBrandImageFromXFile({
    ...
    try {
      ...
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final finalFileName =
          fileName ?? 'brand_$timestamp.${xFile.name.split('.').last}';
      final storagePath = 'brands/$finalFileName';
  ...
  static Future<void> deleteBrandImage(String imageUrl) async {
    try {
      if (kDebugMode) {
  ...
  static Future<String> convertBase64ToUrl(String base64Image) async {
    try {
      ...
      final url = await uploadBrandImage(imageData: base64Image);
