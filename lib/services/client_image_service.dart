...
class ClientImageService {
  ...
  /// رفع صورة زبون إلى Firebase Storage
  static Future<String> uploadClientImage({
    required String imageData,
    String? fileName,
  }) async {
    try {
      if (kDebugMode) {
        print('🔄 Starting client image upload...');
        print(
      ...
      final finalFileName = fileName ?? 'client_$timestamp.jpg';
      final storagePath = 'clients/$finalFileName';
  ...
  /// رفع صورة من XFile
  static Future<String> uploadClientImageFromXFile({
    ...
    try {
      ...
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final finalFileName =
          fileName ?? 'client_$timestamp.${xFile.name.split('.').last}';
      final storagePath = 'clients/$finalFileName';
  ...
  static Future<void> deleteClientImage(String imageUrl) async {
    try {
      if (kDebugMode) {
  ...
  static Future<String> convertBase64ToUrl(String base64Image) async {
    try {
      ...
      final url = await uploadClientImage(imageData: base64Image);
